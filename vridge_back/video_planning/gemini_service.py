...
class GeminiService:
    ...
    def generate_stories_from_planning(self, planning_text):
        prompt = f"""
        당신은 전문 영상 스토리 작가입니다. 아래 기획안을 바탕으로 기승전결 구조의 4개 스토리를 작성해주세요.
        
        반드시 다음 순서로 4개의 스토리를 생성하세요:
        1. 기(起) - 도입부: 상황 설정과 캐릭터 소개
        2. 승(承) - 전개부: 이야기가 본격적으로 전개
        3. 전(轉) - 전환부: 갈등이나 위기, 반전이 일어남
        4. 결(結) - 결말부: 문제 해결과 메시지 전달
        
        각 스토리는 다음 정보를 포함해야 합니다:
        - 스토리 제목
        - 스토리 단계 (기/승/전/결)
        - 주요 등장인물
        - 핵심 내용
