...
class GeminiService:
    ...
    def generate_stories_from_planning(self, planning_text):
        prompt = f"""
        당신은 전문 영상 스토리 작가입니다. 3막 구조(Three-Act Structure)를 활용하여 하나의 통합된 영상 스토리를 4개의 파트로 나누어 작성해주세요.
        
        중요: 각 파트는 전체 스토리의 일부분이어야 하며, 서로 연결되어 하나의 완성된 이야기를 만들어야 합니다.
