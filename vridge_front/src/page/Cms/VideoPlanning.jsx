...
export default function VideoPlanning() {
  ...
  const updateCompletedSteps = () => {
    const completed = []
    if (planningData.planning) completed.push(1)
    if (planningData.stories.length > 0) completed.push(2)
    if (planningData.scenes.length > 0) completed.push(3)
    if (planningData.shots.length > 0) completed.push(4)
    if (planningData.scenes.some(scene => scene.storyboard)) completed.push(5)
    setCompletedSteps(completed)
  }

  // 단계 이동 가능 여부 확인 함수
  const canNavigateToStep = (step) => {
    if (step === 1) return true // 기획안 단계는 항상 이동 가능
    if (step === 2) return !!planningData.planning // 스토리 단계는 기획안이 있어야 이동 가능
    if (step === 3) return planningData.stories.length > 0 // 씬 단계는 스토리가 있어야 이동 가능
    if (step === 4) return planningData.scenes.length > 0 // 샷 단계는 씬이 있어야 이동 가능
    if (step === 5) return planningData.shots.length > 0 // 콘티 단계는 샷이 있어야 이동 가능
    return false
  }
