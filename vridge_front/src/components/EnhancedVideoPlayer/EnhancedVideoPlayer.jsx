...
const EnhancedVideoPlayer = forwardRef(({ 
  ...
  const handleReady = useCallback(() => {
    console.log('[EnhancedVideoPlayer] Player ready!');
    console.log('[EnhancedVideoPlayer] Player ref:', playerRef.current);
    console.log('[EnhancedVideoPlayer] Video URL:', videoUrl);
    
    setIsReady(true);
    setHasError(false);
    setBuffering(false);
    
    if (initialTime > 0 && playerRef.current && typeof playerRef.current.seekTo === 'function') {
      console.log('[EnhancedVideoPlayer] Seeking to initial time:', initialTime);
      playerRef.current.seekTo(initialTime);
    }
    
    onReady?.();
  }, [initialTime, onReady, videoUrl]);
  ...
  const togglePlay = useCallback(() => {
    console.log('[EnhancedVideoPlayer] Toggle play, current playing:', playing);
    ...
    if (playing) {
      ...
      // ReactPlayer가 준비되었는지 확인
      if (playerRef.current && isReady) {
        console.log('[EnhancedVideoPlayer] Starting playback...');
        // 브라우저 자동재생 정책 우회를 위해 약간의 지연 후 재생
        setTimeout(() => {
          if (playerRef.current) {
            const internalPlayer = playerRef.current.getInternalPlayer();
            if (internalPlayer && internalPlayer.play) {
              internalPlayer.play().catch(e => {
                console.error('[EnhancedVideoPlayer] Playback failed:', e);
                // 자동재생 실패 시 음소거 후 재시도
                setMute(true);
                setVolume(0);
                internalPlayer.play().catch(e2 => {
                  console.error('[EnhancedVideoPlayer] Muted playback also failed:', e2);
                });
              });
            }
          }
  ...
  }, [playing, onPlay, onPause, isReady]);
  ...
  const handleProgressClick = useCallback((e) => {
    ...
    // ReactPlayer의 seekTo 메서드를 안전하게 호출
    if (playerRef.current && typeof playerRef.current.seekTo === 'function') {
  ...
  const handleError = useCallback((error) => {
    console.error('[EnhancedVideoPlayer] Video player error:', error);
  ...
  }, [videoUrl, autoplay, muted, width, height, initialTime]);
