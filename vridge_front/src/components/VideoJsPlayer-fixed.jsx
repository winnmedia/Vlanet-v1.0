...
const VideoJsPlayer = forwardRef(({ 
    ...
    play: () => {
      if (playerRef.current) {
        ...
        if (promise !== undefined) {
          promise.catch(e => console.log('[VideoJsPlayer] Play prevented:', e))
        }
        return true
      }
      return false
  ...
  useEffect(() => {
    ...
    console.log('[VideoJsPlayer-fixed] Initializing player with URL:', videoUrl)
    ...
    const player = videojs(videoRef.current, options, function onPlayerReady() {
      console.log('[VideoJsPlayer-fixed] Player is ready')
      ...
      this.on('error', function(e) {
        ...
        console.error('[VideoJsPlayer-fixed] Video error:', error)
        if (onError) {
          onError(error)
        }
      })
      ...
      this.on('loadeddata', function() {
        console.log('[VideoJsPlayer-fixed] Video loaded successfully')
        console.log('Duration:', this.duration())
        console.log('Dimensions:', this.videoWidth(), 'x', this.videoHeight())
      })
    ...
    if (autoplay && player.readyState() >= 2) {
      setTimeout(() => {
        ...
        if (promise !== undefined) {
          promise.catch(e => console.log('[VideoJsPlayer] Autoplay prevented:', e))
        }
      }, 100)
    }
