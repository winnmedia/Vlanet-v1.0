...
### 개선
- 모든 설정 파일 참조를 config.settings.railway로 통일 (70개 이상 파일)
- 오류 방지를 위한 전체 코드베이스 점검 완료
- 프로젝트 생성 기능 로컬 테스트 성공
...
### 주요 수정
- 프로젝트 생성 기능 문제 해결
- Project 모델과 데이터베이스 스키마 동기화
- 마이그레이션 0020 적용 (협업 필드 재추가)
- API 엔드포인트 복구 (users/me, users/refresh, video-planning/create)

### 개선
- 마이그레이션 관리 가이드 CLAUDE.md에 추가
- 설정 파일 구조 문서화 (SETTINGS_GUIDE.md)
- 테스트 스크립트 추가

### 알려진 이슈
- 로컬 SQLite 환경에서 외래키 제약조건 문제
- Railway PostgreSQL 환경에서는 정상 작동 예상
