...
import NativeVideoPlayer from 'components/EnhancedVideoPlayer/NativeVideoPlayer'
...
export default function Feedback() {
  ...
  const captureScreenshot = () => {
    try {
      // NativeVideoPlayer의 getPlayer 메서드로 비디오 엘리먼트 가져오기
      let video = null;
      
      if (videoPlayerRef.current && videoPlayerRef.current.getPlayer) {
        video = videoPlayerRef.current.getPlayer();
      }
      
      // getPlayer가 없거나 실패한 경우 DOM에서 직접 찾기
      if (!video) {
        // 모든 video 엘리먼트 찾기
        const allVideos = document.querySelectorAll('video');
        console.log('[Screenshot] Found video elements:', allVideos.length);
        
        // 보이는 비디오 찾기
        for (const v of allVideos) {
          const rect = v.getBoundingClientRect();
          if (rect.width > 0 && rect.height > 0 && v.src) {
            video = v;
            console.log('[Screenshot] Using visible video:', v.src);
            break;
          }
        }
      }
      ...
      if (!canvas.width || !canvas.height) {
        toast.error('비디오가 아직 로드되지 않았습니다.');
        return;
      }
      ...
      canvas.toBlob((blob) => {
        ...
        a.href = url;
        a.download = `screenshot_${current_project.name || 'project'}_${timestamp.replace(':', '-')}.png`;
        a.click();
        ...
        <main>
            {current_project && (
              <div className={layoutStyles.videoSection}>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <span><strong>담당자:</strong> {current_project.manager}</span>
                        <span><strong>고객사:</strong> {current_project.consumer}</span>
                        <span><strong>상태:</strong> <span style={{
                          color: current_project.project_progress === 'finished' ? '#28a745' : '#1631F8',
                          fontWeight: '600'
                        }}>{current_project.project_progress === 'finished' ? '완료' : '진행중'}</span></span>
                      </div>
                    </div>
                    <button 
                      onClick={() => setShowProjectInfo(!showProjectInfo)}
                      style={{ 
                        background: showProjectInfo ? 'linear-gradient(135deg, #1631F8 0%, #0F23C9 100%)' : 'rgba(108, 117, 125, 0.1)',
                  ...
                  {showProjectInfo && (
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <div>
                          <strong style={{ color: '#495057' }}>생성일:</strong> {moment(current_project.created).format('YYYY.MM.DD')}
                        </div>
                        {current_project.description && (
                          <div style={{ gridColumn: '1 / -1' }}>
                            <strong style={{ color: '#495057' }}>설명:</strong> {current_project.description}
                          </div>
                        )}
                      </div>
                      {is_admin && (
                ...
                {current_project && (
                  <FeedbackList
                    feedbacks={current_project.feedback || []}
                    ...
                    onDelete={(item) => {
                      console.log('Delete feedback:', item);
                      // 삭제 기능 구현
                    }}
                  />
                )}
                  ...
                  <div className="flex space_between">
                    ...
                    <div>
                      ...
                      {currentItem && currentItem.tab === '피드백 관리' && (
                        <button
                          onClick={() => {
                            // Next.js에서는 query 파라미터로 프로젝트 ID 전달
                            navigate(`/FeedbackAll?project_id=${project_id}`)
                          }}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M3 16.5V18.75C3 19.9926 4.00736 21 5.25 21H18.75C19.9926 21 21 19.9926 21 18.75V16.5M16.5 12L12 7.5M12 7.5L7.5 12M12 7.5V16.5" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
