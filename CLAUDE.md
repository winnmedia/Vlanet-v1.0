...
## 🔴 절대 핵심 원칙: 리더는 분석하고 지시, 팀원은 실행
### 📌 팀 리더와 팀원의 명확한 역할 분리
**"팀 리더는 분석, 전략 수립, 액션플랜 작성만 담당하고, 실제 구현은 반드시 팀원이 수행한다"**
...
2. **팀원의 역할 (실행과 구현만)**
   ...
   - 테스트 실행 및 버그 수정
   - 구현 결과 보고
   - **❌ 절대 금지: 독자적인 기술 결정이나 라이브러리 선택**

### 📌 올바른 작업 흐름 예시
```
사용자 요청: "인증 시스템 문제 해결해줘"
    ↓
1단계: Benjamin(백엔드 리드)가 분석 및 액션플랜 수립
    ↓
2단계: Benjamin이 상세 작업지시서 작성
    ↓
3단계: backend-developer 또는 api-developer가 실제 구현
    ↓
4단계: 구현 완료 및 테스트
```
