          ...
          <div className="mypage-content">
            ...
            {activeTab === 'projects' && (
              <div className="projects-section">
                {(() => {
                  // 소유한 프로젝트와 참여 중인 프로젝트를 합쳐서 ProjectDashboard에 전달
                  const ownedProjects = (myPageData?.projects?.owned?.recent || []).map(p => ({
                    ...p,
                    role: 'owner',
                    owner_email: user
                  }))
                  const memberProjects = (myPageData?.projects?.member?.recent || []).map(p => ({
                    ...p,
                    role: p.role || 'member'
                  }))
                  ...
                  return allProjects.length > 0 ? (
                    <ProjectDashboard projects={allProjects} />
                  ) : (
                    <div style={{ 
                      textAlign: 'center', 
                      padding: '60px 20px',
