...
/**
 * VideoPlanet 디자인 일치율 분석 도구 v3
 * 
 * 개선사항:
 * 1. 통합 Button 컴포넌트 import 패턴 정확히 감지
 * 2. JSX/TSX 파일에서 Button 사용 패턴 개선
 * 3. 더 정확한 컴포넌트 통계
 */

class DesignConsistencyAnalyzerV3 {
  constructor() {
    this.results = {
      brandColorUsage: {},
      hardcodedValues: {
  ...
  async analyze() {
    console.log('🎨 VideoPlanet 디자인 일치율 분석 v3 시작...\n');
  ...
  async analyzeComponents() {
    ...
    for (const file of componentFiles) {
      ...
      // Button 분석 개선
      const hasUnifiedButtonImport = 
        content.includes("import { Button } from") && content.includes("/unified/Button") ||
        content.includes("import Button from") && content.includes("/unified/Button");
      
      if (hasUnifiedButtonImport) {
        // Unified Button을 import하고 있다면, 이 파일의 모든 <Button>은 unified
        const buttonMatches = content.match(/<Button[\s>]/g) || [];
        componentUsage.buttons.unified += buttonMatches.length;
        if (buttonMatches.length > 0) {
          componentUsage.buttons.files.unified.push(relativePath);
        }
      } else {
        // Unified Button을 import하지 않는 경우
        const htmlButtonMatches = content.match(/<button[\s>]/g) || [];
        const customButtonMatches = content.match(/<Button[\s>]/g) || [];
        const totalCustom = htmlButtonMatches.length + customButtonMatches.length;
        
        componentUsage.buttons.custom += totalCustom;
        if (totalCustom > 0) {
          componentUsage.buttons.files.custom.push(relativePath);
        }
      }
      
      // Input 분석
      const hasUnifiedInputImport = 
        content.includes("import { Input } from") && content.includes("/unified/Input") ||
        content.includes("import Input from") && content.includes("/unified/Input");
      
      if (hasUnifiedInputImport) {
        const inputMatches = content.match(/<Input[\s>]/g) || [];
        componentUsage.inputs.unified += inputMatches.length;
        if (inputMatches.length > 0) {
          componentUsage.inputs.files.unified.push(relativePath);
        }
      } else {
        const htmlInputMatches = content.match(/<input[\s>]/g) || [];
        const customInputMatches = content.match(/<Input[\s>]/g) || [];
        const totalCustom = htmlInputMatches.length + customInputMatches.length;
        
        componentUsage.inputs.custom += totalCustom;
        if (totalCustom > 0) {
          componentUsage.inputs.files.custom.push(relativePath);
        }
      }
      
      // Card 분석
      const hasUnifiedCardImport = 
        content.includes("import { Card } from") && content.includes("/unified/Card") ||
        content.includes("import Card from") && content.includes("/unified/Card");
      
      if (hasUnifiedCardImport) {
        const cardMatches = content.match(/<Card[\s>]/g) || [];
        componentUsage.cards.unified += cardMatches.length;
      } else if (content.includes('<Card')) {
        const cardMatches = content.match(/<Card[\s>]/g) || [];
        componentUsage.cards.custom += cardMatches.length;
      }
      
      // Modal 분석
      const hasUnifiedModalImport = 
        content.includes("import { Modal } from") && content.includes("/unified/Modal") ||
        content.includes("import Modal from") && content.includes("/unified/Modal");
      
      if (hasUnifiedModalImport) {
        const modalMatches = content.match(/<Modal[\s>]/g) || [];
        componentUsage.modals.unified += modalMatches.length;
      } else if (content.includes('<Modal')) {
        const modalMatches = content.match(/<Modal[\s>]/g) || [];
        componentUsage.modals.custom += modalMatches.length;
      }
    }
    ...
    // 디버그 정보 출력
    console.log(`  - 버튼: ${componentUsage.buttons.unified} unified, ${componentUsage.buttons.custom} custom`);
    console.log(`  - 입력: ${componentUsage.inputs.unified} unified, ${componentUsage.inputs.custom} custom`);
    
    // 통계 저장
    this.results.componentStats = componentUsage;
  ...
  generateReport() {
    ...
    // 리포트 파일 저장
    fs.writeFileSync(
      'design-consistency-report-v3.json',
    ...
    console.log(`- 카드 일관성: ${report.details.componentConsistency.cards}%`);
    console.log(`- 모달 일관성: ${report.details.componentConsistency.modals}%`);
    
    if (this.results.componentStats) {
      console.log('\n컴포넌트 통계:');
      console.log(`- 통합 Button 사용: ${this.results.componentStats.buttons.unified}개`);
      console.log(`- 커스텀 버튼 사용: ${this.results.componentStats.buttons.custom}개`);
    }
    ...
    console.log('\n✅ 상세 리포트가 design-consistency-report-v3.json에 저장되었습니다.');
  }
...
if (require.main === module) {
  const analyzer = new DesignConsistencyAnalyzerV3();
  analyzer.analyze().catch(console.error);
}

module.exports = DesignConsistencyAnalyzerV3;